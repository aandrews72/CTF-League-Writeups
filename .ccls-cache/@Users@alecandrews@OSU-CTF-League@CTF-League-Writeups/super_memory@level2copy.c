...
int main() {
    ...
    // Initialize struct with known values
    player_info.jumpCount = 1;
    player_info.collected_items = 0;
    player_info.xpos = 0;
    player_info.ypos = 0;
    player_info.dollars = 123456789; // Set to a known value
    memset(player_info.inventory, 'X', sizeof(player_info.inventory)); // Fill inventory with 'X'

    // Print offsets and sizes
    printf("Struct PlayerInfo size: %zu bytes\n", sizeof(struct PlayerInfo));
    printf("Offset of jumpCount: %zu bytes\n", offsetof(struct PlayerInfo, jumpCount));
    printf("Offset of collected_items: %zu bytes\n", offsetof(struct PlayerInfo, collected_items));
    printf("Offset of xpos: %zu bytes\n", offsetof(struct PlayerInfo, xpos));
    printf("Offset of ypos: %zu bytes\n", offsetof(struct PlayerInfo, ypos));
    printf("Offset of inventory: %zu bytes\n", offsetof(struct PlayerInfo, inventory));
    printf("Offset of dollars: %zu bytes\n", offsetof(struct PlayerInfo, dollars));

    // Pause the program to inspect in GDB
    printf("Press Enter to continue...");
    getchar(); // Wait for input

    // Simulate collecting items beyond the inventory capacity
    char items_to_collect[] = {'a', 'b', 'c', 'd', 'e', 'f', 'g'};
    for (int i = 0; i < sizeof(items_to_collect); i++) {
        player_info.inventory[player_info.collected_items] = items_to_collect[i];
        player_info.collected_items++;
    }

    // Print the values after overflow
    printf("After collecting items:\n");
    printf("collected_items: %d\n", player_info.collected_items);
    printf("inventory: ");
    for (int i = 0; i < sizeof(player_info.inventory); i++) {
        printf("%c ", player_info.inventory[i]);
    }
    printf("\n");
    printf("dollars: %u\n", player_info.dollars);

    // Pause again to inspect in GDB
